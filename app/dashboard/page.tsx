...
export default function DashboardPage() {
  ...
  return (
    <div className="grid min-w-0 gap-12 lg:gap-14">
      <section className="relative isolate grid min-h-[calc(100svh-96px)] w-full min-w-0 content-start overflow-hidden rounded-lg border border-slate-800/90 bg-slate-950/42 p-4 shadow-[0_24px_70px_rgba(2,6,23,0.28)] sm:p-5">
        <div
          className="pointer-events-none absolute inset-0 bg-[linear-gradient(120deg,rgba(79,70,229,0.15),transparent_34%),linear-gradient(245deg,rgba(6,182,212,0.12),transparent_38%)]"
          aria-hidden="true"
        />
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-16 bg-gradient-to-b from-transparent via-slate-950/40 to-slate-950" aria-hidden="true" />
        ...
        <div className="relative grid min-w-0 gap-4">
          <div className="grid min-w-0 gap-4 lg:grid-cols-[minmax(0,1fr)_224px] lg:items-start">
            <div className="min-w-0 rounded-lg border border-slate-800/80 bg-slate-900/66 p-5 shadow-[0_18px_44px_rgba(2,6,23,0.18)]">
              ...
              <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300 md:text-base md:leading-7">
                Acompanhe recebimentos, saldo, saques e performance Pix em uma visao limpa da {accountOverview.platformName}.
              </p>
            ...
            <div className="hidden rounded-lg border border-slate-800/80 bg-slate-900/58 p-3 shadow-[0_18px_44px_rgba(2,6,23,0.18)] lg:grid">
              <SparkSpotlight
                variant="dark"
              ...
              />
              <div className="mt-3 grid gap-1.5 rounded-lg border border-cyan-400/15 bg-cyan-400/10 p-2.5">
                <p className="text-xs font-extrabold uppercase tracking-[0.14em] text-cyan-200">Spark monitor</p>
                <p className="text-xs leading-5 text-slate-300">Alertas, Pix aprovados e automacoes em foco.</p>
              </div>
            </div>
          </div>

          <div className="flex flex-col gap-3 rounded-lg border border-slate-800/80 bg-slate-950/40 p-2.5 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-wrap gap-2">
              {periodOptions.map((period, index) => (
                <button
                  key={period}
                  type="button"
                  className={`rounded-lg px-3 py-2 text-sm font-extrabold transition duration-300 ${
                    index === 0
                      ? "bg-cyan-400 text-slate-950 shadow-[0_12px_24px_rgba(6,182,212,0.2)]"
                      : "border border-slate-800 text-slate-300 hover:border-cyan-400/60 hover:bg-slate-900 hover:text-white"
                  }`}
                >
                  {period}
                </button>
        ...
        <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm leading-6 text-slate-400">
              Tudo que o cliente usa com mais frequencia no dia a dia da gateway, separado da visao geral para manter foco.
            </p>
          </div>
        </div>
      ...
      <section className="dashboard-deferred grid min-w-0 scroll-mt-24 gap-6 xl:grid-cols-[minmax(0,1.55fr)_minmax(320px,0.85fr)]">
        <div className="overflow-hidden rounded-lg border border-slate-800 bg-slate-900/66 shadow-[0_18px_46px_rgba(2,6,23,0.18)]">
          <div className="flex flex-col gap-4 border-b border-slate-800 p-5 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-sm font-bold uppercase tracking-[0.14em] text-cyan-300">Analise e performance</p>
